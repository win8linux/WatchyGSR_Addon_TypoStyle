...
RTC_DATA_ATTR int MyGSRWatchFaceTypoStyleId; // Watchface ID #.
// Approx. 3035 bytes

class GSRWatchFaceTypoStyle : public WatchyGSR {
public:
    GSRWatchFaceTypoStyle()
        : WatchyGSR()
    {
        initAddOn(this);
    } // *** DO NOT EDIT THE CONTENTS OF THIS FUNCTION ***

    TypoStyle TypoStyleyWatchFace;

    void RegisterWatchFaces()
    { // Add WatchStyles here (this is done pre-boot, post init)
        MyGSRWatchFaceTypoStyleId = AddWatchStyle("TypoStyle", this);
    };
    ...
    void InsertDrawWatchStyle(uint8_t StyleID)
    {
        uint8_t X, Y;
        uint16_t A;
        if (StyleID == MyGSRWatchFaceTypoStyleId) {
            if (SafeToDraw()) {
                TypoStyleyWatchFace.drawWatchFace();
            }
        }
    };
};

GSRWatchFaceTypoStyle MyGSRWatchFaceTypoStyle;
