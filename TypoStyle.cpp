...
void drawPixel(int16_t x, int16_t y, uint16_t col)
{
    ...
    case GREY:
        if (y & 1) {
            if (x & 1) {
                WatchyGSR::display.drawPixel(x, y, watchyGSR.BackColor());
            } else {
                WatchyGSR::display.drawPixel(x, y, watchyGSR.ForeColor());
            }
        } else {
            if (x & 1) {
                WatchyGSR::display.drawPixel(x, y, watchyGSR.ForeColor());
        ...
        break;
    default:
        WatchyGSR::display.drawPixel(x, y, col);
        break;
    }
}
...
{ // override this method to customize how the watch face looks
    ...
    int clockLeft = -14;
    int clockTop = 15;
    int hour1 = WatchTime.Local.Hour / 10;
    int hour2 = WatchTime.Local.Hour % 10;
    int minute1 = WatchTime.Local.Minute / 10;
    int minute2 = WatchTime.Local.Minute % 10;
    int month = WatchTime.Local.Month - 1;
    int day1 = WatchTime.Local.Day / 10;
    int day2 = WatchTime.Local.Day % 10;
    int year = WatchTime.Local.Year + 1970;
    int weekday = WatchTime.Local.Wday - 1;
    ...
    int col1 = watchyGSR.ForeColor(); // foreground colour
    int col2 = watchyGSR.BackColor(); // background colour
